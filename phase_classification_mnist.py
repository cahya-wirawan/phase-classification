...
from kaggle_mnist import custom_kaggle_mnist

# define baseline model
def baseline_model(dropout=0.1):
    input_bhe = Input(shape=(28, 28, 1), name='input_bhe')
    ...
    model_flatten = Flatten()(model_bhe)
    ...
    model_fc = Dense(64, activation='relu')(model_fc)
    model_fc = Dropout(dropout)(model_fc)
    output = Dense(10, activation='softmax', name='output')(model_fc)
    model = Model(inputs=input_bhe, outputs=output)
...
if __name__ == "__main__":
    ...
    if args.action == "train":
        ...
        if args.cv:
            ...
            results = cross_val_score(estimator, mnist["train_x"], mnist["train_y"], cv=kfold,
